...
//#include "stdafx.h"  NOT USING PRECOMPILED HEADERS
#include "CSmtp.h"
#include "base64.h"
...
{
	{SMTP_COMMAND::command_INIT,          0,     5*60,  220, ECSmtp::CSmtpError::SERVER_NOT_RESPONDING},
	{SMTP_COMMAND::command_EHLO,          5*60,  5*60,  250, ECSmtp::CSmtpError::COMMAND_EHLO},
	{SMTP_COMMAND::command_AUTHPLAIN,     5*60,  5*60,  235, ECSmtp::CSmtpError::COMMAND_AUTH_PLAIN},
	{SMTP_COMMAND::command_AUTHLOGIN,     5*60,  5*60,  334, ECSmtp::CSmtpError::COMMAND_AUTH_LOGIN},
	{SMTP_COMMAND::command_AUTHCRAMMD5,   5*60,  5*60,  334, ECSmtp::CSmtpError::COMMAND_AUTH_CRAMMD5},
	{SMTP_COMMAND::command_AUTHDIGESTMD5, 5*60,  5*60,  334, ECSmtp::CSmtpError::COMMAND_AUTH_DIGESTMD5},
	{SMTP_COMMAND::command_DIGESTMD5,     5*60,  5*60,  335, ECSmtp::CSmtpError::COMMAND_DIGESTMD5},
	{SMTP_COMMAND::command_USER,          5*60,  5*60,  334, ECSmtp::CSmtpError::UNDEF_XYZ_RESPONSE},
	{SMTP_COMMAND::command_PASSWORD,      5*60,  5*60,  235, ECSmtp::CSmtpError::BAD_LOGIN_PASS},
	{SMTP_COMMAND::command_MAILFROM,      5*60,  5*60,  250, ECSmtp::CSmtpError::COMMAND_MAIL_FROM},
	{SMTP_COMMAND::command_RCPTTO,        5*60,  5*60,  250, ECSmtp::CSmtpError::COMMAND_RCPT_TO},
	{SMTP_COMMAND::command_DATA,          5*60,  2*60,  354, ECSmtp::CSmtpError::COMMAND_DATA},
	{SMTP_COMMAND::command_DATABLOCK,     3*60,  0,     0,   ECSmtp::CSmtpError::COMMAND_DATABLOCK},	// Here the valid_reply_code is set to zero because there are no replies when sending data blocks
	{SMTP_COMMAND::command_DATAEND,       3*60,  10*60, 250, ECSmtp::CSmtpError::MSG_BODY_ERROR},
	{SMTP_COMMAND::command_QUIT,          5*60,  5*60,  221, ECSmtp::CSmtpError::COMMAND_QUIT},
	{SMTP_COMMAND::command_STARTTLS,      5*60,  5*60,  220, ECSmtp::CSmtpError::COMMAND_EHLO_STARTTLS}
};
...
CSmtp::CSmtp()
{
	...
	m_bConnected = false;
	m_iXPriority = CSmptXPriority::XPRIORITY_NORMAL;
	m_iSMTPSrvPort = 0;
	...
	if (WSAStartup(wVer,&wsaData) != NO_ERROR)
		throw ECSmtp(ECSmtp::CSmtpError::WSA_STARTUP);
	if (LOBYTE( wsaData.wVersion ) != 2 || HIBYTE( wsaData.wVersion ) != 2 ) 
	{
		WSACleanup();
		throw ECSmtp(ECSmtp::CSmtpError::WSA_VER);
	}
#endif

	char hostname[255];
	if(gethostname((char *) &hostname, 255) == SOCKET_ERROR) throw ECSmtp(ECSmtp::CSmtpError::WSA_HOSTNAME);
	m_sLocalHostName = hostname;
	
	if((RecvBuf = new char[BUFFER_SIZE]) == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::LACK_OF_MEMORY);
	
	if((SendBuf = new char[BUFFER_SIZE]) == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::LACK_OF_MEMORY);

	m_type = SMTP_SECURITY_TYPE::NO_SECURITY;
	m_ctx = NULL;
	m_ssl = NULL;
	m_bHTML = false;
...
{	
	if(!email)
		throw ECSmtp(ECSmtp::CSmtpError::UNDEF_RECIPIENT_MAIL);
...
void CSmtp::DelMsgLine(unsigned int Line)
{
	if(Line >= MsgBody.size())
		throw ECSmtp(ECSmtp::CSmtpError::OUT_OF_MSG_RANGE);
...
void CSmtp::Send()
{
	unsigned int rcpt_count, res, FileId;
	...
	if(hSocket==INVALID_SOCKET)
	{
		if(!ConnectRemoteServer(m_sSMTPSrvName.c_str(), m_iSMTPSrvPort, m_type, m_bAuthenticate))
			throw ECSmtp(ECSmtp::CSmtpError::WSA_INVALID_SOCKET);
	}
	...
	try{
		...
		{
			// opening the file:
			err = fopen_s(&hFile, Attachments[FileId].c_str(), "rb");
			if(err != 0)
				throw ECSmtp(ECSmtp::CSmtpError::FILE_NOT_EXIST);
			...
			if(TotalSize/1024 > MSG_SIZE_IN_MB*1024)
				throw ECSmtp(ECSmtp::CSmtpError::MSG_TOO_BIG);
		...
		if(!m_sMailFrom.size())
			throw ECSmtp(ECSmtp::CSmtpError::UNDEF_MAIL_FROM);
		Command_Entry* pEntry = FindCommandEntry(SMTP_COMMAND::command_MAILFROM);
		...
		if(!(rcpt_count = Recipients.size()))
			throw ECSmtp(ECSmtp::CSmtpError::UNDEF_RECIPIENTS);
		pEntry = FindCommandEntry(SMTP_COMMAND::command_RCPTTO);
		for (auto& Recipient : Recipients)
		{
			snprintf(SendBuf, BUFFER_SIZE, "RCPT TO:<%s>\r\n", (Recipient.Mail).c_str());
			SendData(pEntry);
			ReceiveResponse(pEntry);
		}

		for (auto& CCRecipient : CCRecipients)
		{
			snprintf(SendBuf, BUFFER_SIZE, "RCPT TO:<%s>\r\n", (CCRecipient.Mail).c_str());
			SendData(pEntry);
			ReceiveResponse(pEntry);
		}

		for (auto& BCCRecipient : BCCRecipients)
		{
			snprintf(SendBuf, BUFFER_SIZE, "RCPT TO:<%s>\r\n", (BCCRecipient.Mail).c_str());
			SendData(pEntry);
			ReceiveResponse(pEntry);
		}
		
		pEntry = FindCommandEntry(SMTP_COMMAND::command_DATA);
		// DATA <CRLF>
		snprintf(SendBuf, BUFFER_SIZE, "DATA\r\n");
		SendData(pEntry);
		ReceiveResponse(pEntry);
		
		pEntry = FindCommandEntry(SMTP_COMMAND::command_DATABLOCK);
		// send header(s)
		...
		for(FileId = 0; FileId < Attachments.size(); FileId++)
		{
			...
			// opening the file:
			err = fopen_s(&hFile, FileName.c_str(), "rb");
			if(err != 0)
				throw ECSmtp(ECSmtp::CSmtpError::FILE_NOT_EXIST);
		...
		pEntry = FindCommandEntry(SMTP_COMMAND::command_DATAEND);
		// <CRLF> . <CRLF>
		snprintf(SendBuf, BUFFER_SIZE, "\r\n.\r\n");
		SendData(pEntry);
		ReceiveResponse(pEntry);
	...
	try
	{
		...
		if((sockAddr.sin_addr.s_addr = inet_addr(szServer)) == INADDR_NONE)
		{
			...
			else
			{
				...
				closesocket(hSocket);
#endif
				throw ECSmtp(ECSmtp::CSmtpError::WSA_GETHOSTBY_NAME_ADDR);
			}				
		}
...
#endif
		{
			...
			closesocket(hSocket);
#endif
			throw ECSmtp(ECSmtp::CSmtpError::WSA_IOCTLSOCKET);
		}
		...
		if(connect(hSocket,(LPSOCKADDR)&sockAddr,sizeof(sockAddr)) == SOCKET_ERROR)
		{
...
#endif
			{
				...
				closesocket(hSocket);
#endif
				throw ECSmtp(ECSmtp::CSmtpError::WSA_CONNECT);
			}
		}
		else
			return true;
		...
		while(true)
		{
			...
			if((res = select(hSocket+1,NULL,&fdwrite,&fdexcept,&timeout)) == SOCKET_ERROR)
			{
				...
				closesocket(hSocket);
#endif
				throw ECSmtp(ECSmtp::CSmtpError::WSA_SELECT);
			}
			...
			if(!res)
			{
				...
				closesocket(hSocket);
#endif
				throw ECSmtp(ECSmtp::CSmtpError::SELECT_TIMEOUT);
			}
		...
		if(securityType!=SMTP_SECURITY_TYPE::DO_NOT_SET) SetSecurityType(securityType);
		if(GetSecurityType() == SMTP_SECURITY_TYPE::USE_TLS || GetSecurityType() == SMTP_SECURITY_TYPE::USE_SSL)
		{
			InitOpenSSL();
			if(GetSecurityType() == SMTP_SECURITY_TYPE::USE_SSL)
			{
				OpenSSLConnect();
			}
		}

		Command_Entry* pEntry = FindCommandEntry(SMTP_COMMAND::command_INIT);
		ReceiveResponse(pEntry);

		SayHello();

		if(GetSecurityType() == SMTP_SECURITY_TYPE::USE_TLS)
		{
			StartTls();
			SayHello();
		}
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			if(!m_sLogin.size())
				throw ECSmtp(ECSmtp::CSmtpError::UNDEF_LOGIN);

			if(password) SetPassword(password);
			if(!m_sPassword.size())
				throw ECSmtp(ECSmtp::CSmtpError::UNDEF_PASSWORD);

			if(IsKeywordSupported(RecvBuf, "LOGIN") == true)
			{
				pEntry = FindCommandEntry(SMTP_COMMAND::command_AUTHLOGIN);
				...
				pEntry = FindCommandEntry(SMTP_COMMAND::command_USER);
				snprintf(SendBuf, BUFFER_SIZE, "%s\r\n",encoded_login.c_str());
				SendData(pEntry);
				ReceiveResponse(pEntry);
				...
				pEntry = FindCommandEntry(SMTP_COMMAND::command_PASSWORD);
			...
			{
				pEntry = FindCommandEntry(SMTP_COMMAND::command_AUTHPLAIN);
			...
			{
				pEntry = FindCommandEntry(SMTP_COMMAND::command_AUTHCRAMMD5);
				...
				if(!ustrChallenge || !ustrPassword)
					throw ECSmtp(ECSmtp::CSmtpError::BAD_LOGIN_PASSWORD);
			...
			{
				pEntry = FindCommandEntry(SMTP_COMMAND::command_DIGESTMD5);
				...
				if(find<0)
					throw ECSmtp(ECSmtp::CSmtpError::BAD_DIGEST_RESPONSE);
				...
				if(!getpeername(hSocket, (struct sockaddr*)&addr, &len))
					throw ECSmtp(ECSmtp::CSmtpError::BAD_SERVER_NAME);
			...
			else throw ECSmtp(ECSmtp::CSmtpError::LOGIN_NOT_SUPPORTED);
		}
	}
	catch(const ECSmtp&)
	{
...
void CSmtp::FormatHeader(char* header)
{
	...
	if(err != 0)
		throw ECSmtp(ECSmtp::CSmtpError::TIME_ERROR);
	...
	if(!m_sMailFrom.size()) throw ECSmtp(ECSmtp::CSmtpError::UNDEF_MAIL_FROM);
	...
	switch(m_iXPriority)
	{
		case CSmptXPriority::XPRIORITY_HIGH:
			strcat_s(header, BUFFER_SIZE, "X-Priority: 2 (High)\r\n");
			break;
		case CSmptXPriority::XPRIORITY_NORMAL:
			strcat_s(header, BUFFER_SIZE, "X-Priority: 3 (Normal)\r\n");
			break;
		case CSmptXPriority::XPRIORITY_LOW:
...
void CSmtp::ReceiveData(Command_Entry* pEntry)
{
	...
	if(RecvBuf == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::RECVBUF_IS_EMPTY);
	...
	if(!res)
	{
		//timeout
		FD_CLR(hSocket,&fdread);
		throw ECSmtp(ECSmtp::CSmtpError::SERVER_NOT_RESPONDING);
	}
	...
	if(FD_ISSET(hSocket,&fdread))
	{
		...
		{
			FD_CLR(hSocket,&fdread);
			throw ECSmtp(ECSmtp::CSmtpError::WSA_RECV);
		}
	}
	...
	if(res == 0)
	{
		throw ECSmtp(ECSmtp::CSmtpError::CONNECTION_CLOSED);
	}
}
...
void CSmtp::SendData(Command_Entry* pEntry)
{
	...
	if(SendBuf == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::SENDBUF_IS_EMPTY);
	...
	while(nLeft > 0)
	{
		...
		if(res && FD_ISSET(hSocket,&fdwrite))
		{
			...
			{
				FD_CLR(hSocket,&fdwrite);
				throw ECSmtp(ECSmtp::CSmtpError::WSA_SEND);
			}
			nLeft -= res;
			idx += res;
		}
...
std::string ECSmtp::GetErrorText() const
{
	switch(ErrorCode)
	{
		case ECSmtp::CSmtpError::CSMTP_NO_ERROR:
			return "";
		case ECSmtp::CSmtpError::WSA_STARTUP:
			return "Unable to initialise winsock2";
		case ECSmtp::CSmtpError::WSA_VER:
			return "Wrong version of the winsock2";
		case ECSmtp::CSmtpError::WSA_SEND:
			return "Function send() failed";
		case ECSmtp::CSmtpError::WSA_RECV:
			return "Function recv() failed";
		case ECSmtp::CSmtpError::WSA_CONNECT:
			return "Function connect failed";
		case ECSmtp::CSmtpError::WSA_GETHOSTBY_NAME_ADDR:
			return "Unable to determine remote server";
		case ECSmtp::CSmtpError::WSA_INVALID_SOCKET:
			return "Invalid winsock2 socket";
		case ECSmtp::CSmtpError::WSA_HOSTNAME:
			return "Function hostname() failed";
		case ECSmtp::CSmtpError::WSA_IOCTLSOCKET:
			return "Function ioctlsocket() failed";
		case ECSmtp::CSmtpError::BAD_IPV4_ADDR:
			return "Improper IPv4 address";
		case ECSmtp::CSmtpError::UNDEF_MSG_HEADER:
			return "Undefined message header";
		case ECSmtp::CSmtpError::UNDEF_MAIL_FROM:
			return "Undefined mail sender";
		case ECSmtp::CSmtpError::UNDEF_SUBJECT:
			return "Undefined message subject";
		case ECSmtp::CSmtpError::UNDEF_RECIPIENTS:
			return "Undefined at least one reciepent";
		case ECSmtp::CSmtpError::UNDEF_RECIPIENT_MAIL:
			return "Undefined recipent mail";
		case ECSmtp::CSmtpError::UNDEF_LOGIN:
			return "Undefined user login";
		case ECSmtp::CSmtpError::UNDEF_PASSWORD:
			return "Undefined user password";
		case ECSmtp::CSmtpError::BAD_LOGIN_PASSWORD:
			return "Invalid user login or password";
		case ECSmtp::CSmtpError::BAD_DIGEST_RESPONSE:
			return "Server returned a bad digest MD5 response";
		case ECSmtp::CSmtpError::BAD_SERVER_NAME:
			return "Unable to determine server name for digest MD5 response";
		case ECSmtp::CSmtpError::COMMAND_MAIL_FROM:
			return "Server returned error after sending MAIL FROM";
		case ECSmtp::CSmtpError::COMMAND_EHLO:
			return "Server returned error after sending EHLO";
		case ECSmtp::CSmtpError::COMMAND_AUTH_PLAIN:
			return "Server returned error after sending AUTH PLAIN";
		case ECSmtp::CSmtpError::COMMAND_AUTH_LOGIN:
			return "Server returned error after sending AUTH LOGIN";
		case ECSmtp::CSmtpError::COMMAND_AUTH_CRAMMD5:
			return "Server returned error after sending AUTH CRAM-MD5";
		case ECSmtp::CSmtpError::COMMAND_AUTH_DIGESTMD5:
			return "Server returned error after sending AUTH DIGEST-MD5";
		case ECSmtp::CSmtpError::COMMAND_DIGESTMD5:
			return "Server returned error after sending MD5 DIGEST";
		case ECSmtp::CSmtpError::COMMAND_DATA:
			return "Server returned error after sending DATA";
		case ECSmtp::CSmtpError::COMMAND_QUIT:
			return "Server returned error after sending QUIT";
		case ECSmtp::CSmtpError::COMMAND_RCPT_TO:
			return "Server returned error after sending RCPT TO";
		case ECSmtp::CSmtpError::MSG_BODY_ERROR:
			return "Error in message body";
		case ECSmtp::CSmtpError::CONNECTION_CLOSED:
			return "Server has closed the connection";
		case ECSmtp::CSmtpError::SERVER_NOT_READY:
			return "Server is not ready";
		case ECSmtp::CSmtpError::SERVER_NOT_RESPONDING:
			return "Server not responding";
		case ECSmtp::CSmtpError::FILE_NOT_EXIST:
			return "Attachment file does not exist";
		case ECSmtp::CSmtpError::MSG_TOO_BIG:
			return "Message is too big";
		case ECSmtp::CSmtpError::BAD_LOGIN_PASS:
			return "Bad login or password";
		case ECSmtp::CSmtpError::UNDEF_XYZ_RESPONSE:
			return "Undefined xyz SMTP response";
		case ECSmtp::CSmtpError::LACK_OF_MEMORY:
			return "Lack of memory";
		case ECSmtp::CSmtpError::TIME_ERROR:
			return "time() error";
		case ECSmtp::CSmtpError::RECVBUF_IS_EMPTY:
			return "RecvBuf is empty";
		case ECSmtp::CSmtpError::SENDBUF_IS_EMPTY:
			return "SendBuf is empty";
		case ECSmtp::CSmtpError::OUT_OF_MSG_RANGE:
			return "Specified line number is out of message size";
		case ECSmtp::CSmtpError::COMMAND_EHLO_STARTTLS:
			return "Server returned error after sending STARTTLS";
		case ECSmtp::CSmtpError::SSL_PROBLEM1:
			return "SSL problem 1";
		case ECSmtp::CSmtpError::SSL_PROBLEM2:
			return "SSL problem 2";
		case ECSmtp::CSmtpError::SSL_PROBLEM3:
			return "SSL problem 3";
		case ECSmtp::CSmtpError::SSL_PROBLEM4:
			return "SSL problem 4";
		case ECSmtp::CSmtpError::SSL_PROBLEM5:
			return "SSL problem 5";
		case ECSmtp::CSmtpError::SSL_PROBLEM6:
			return "SSL problem 6";
		case ECSmtp::CSmtpError::COMMAND_DATABLOCK:
			return "Failed to send data block";
		case ECSmtp::CSmtpError::STARTTLS_NOT_SUPPORTED:
			return "The STARTTLS command is not supported by the server";
		case ECSmtp::CSmtpError::LOGIN_NOT_SUPPORTED:
...
void CSmtp::SayHello()
{
	Command_Entry* pEntry = FindCommandEntry(SMTP_COMMAND::command_EHLO);
...
void CSmtp::SayQuit()
{
	...
	Command_Entry* pEntry = FindCommandEntry(SMTP_COMMAND::command_QUIT);
	// QUIT <CRLF>
...
void CSmtp::StartTls()
{
	...
	{
		throw ECSmtp(ECSmtp::CSmtpError::STARTTLS_NOT_SUPPORTED);
	}
	Command_Entry* pEntry = FindCommandEntry(SMTP_COMMAND::command_STARTTLS);
		...
		if(FD_ISSET(hSocket,&fdread) || (read_blocked_on_write && FD_ISSET(hSocket,&fdwrite)) )
		{
			while(1)
			{
				...
				else
				{
					FD_ZERO(&fdread);
					FD_ZERO(&fdwrite);
					throw ECSmtp(ECSmtp::CSmtpError::SSL_PROBLEM1);
				}
			}
		}
	}
		...
		if(FD_ISSET(hSocket,&fdwrite) || (write_blocked_on_read && FD_ISSET(hSocket, &fdread)) )
		{
			...
			switch(SSL_get_error(ssl,res))
			{
			  ...
			  default:	      
				FD_ZERO(&fdread);
				FD_ZERO(&fdwrite);
				throw ECSmtp(ECSmtp::CSmtpError::SSL_PROBLEM2);
			}
...
void CSmtp::InitOpenSSL()
{
	...
	if(m_ctx == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::SSL_PROBLEM3);
}

void CSmtp::OpenSSLConnect()
{
	if(m_ctx == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::SSL_PROBLEM4);
	m_ssl = SSL_new (m_ctx);   
	if(m_ssl == NULL)
		throw ECSmtp(ECSmtp::CSmtpError::SSL_PROBLEM5);
	...
	while(1)
	{
		...
		switch(SSL_get_error(m_ssl, res))
		{
		  ...
		  default:	      
			FD_ZERO(&fdwrite);
			FD_ZERO(&fdread);
			throw ECSmtp(ECSmtp::CSmtpError::SSL_PROBLEM6);
		}
	}
}
